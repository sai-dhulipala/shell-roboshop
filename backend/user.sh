...
# Step 9: Setup SystemD User Service
cp $UTILS_DIR/user.service /etc/systemd/system/user.service &>> $LOG_FILE
echo -e "Setting up SystemD User Service ... ${G}SUCCESS${N}" | tee -a $LOG_FILE
...
echo -e "Enabling user ... ${G}SUCCESS${N}" | tee -a $LOG_FILE

systemctl start user &>> $LOG_FILE
echo -e "Starting user ... ${G}SUCCESS${N}" | tee -a $LOG_FILE
